...
function Resolve-TradingLabRuntimeVerificationState(
    ...
    $checks = @(
        ...
        [pscustomobject]@{ name = 'ACL_ROOT_INHERITANCE_PROTECTED'; pass = [bool](Get-TradingLabProperty $AclAudit 'root_inheritance_protected') },
        [pscustomobject]@{ name = 'ACL_DESCENDANT_POLICY_SAFE'; pass = [bool](Get-TradingLabProperty $AclAudit 'descendant_policy_safe') },
        [pscustomobject]@{ name = 'ACL_UNSAFE_DESCENDANTS_ZERO'; pass = (Get-TradingLabProperty $AclAudit 'unsafe_descendants') -eq 0 },
